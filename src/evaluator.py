...
2. **hook_strength** (Gancho de Copywriting antes de 'Ver más' - máx 220 caracteres):
   - 5: Excelente. Inicia en las primeras 2 líneas con tensión técnica real, contraste o el desafío de arquitectura genuino. Supera el test de copywriting "Now you can..." (nombra una habilidad o resultado concreto nuevo). Cero signos de exclamación y cero rodeos.
   - 3: Aceptable pero genérico o demasiado ingenioso sin claridad inmediata.
   - 1: Pésimo. Pregunta retórica cliché ("¿Alguna vez te has preguntado...?"), saludo, divagación vaga o signos de exclamación forzados.

3. **mobile_readability** (Formato Mobile-First):
   - 5: Perfecto. Párrafos de máximo 2 a 3 líneas con líneas en blanco obligatorias entre párrafos. Oraciones activas y directas.
   - 3: Párrafos algo largos (4-5 líneas).
   - 1: Bloque de texto denso ilegible en pantallas móviles.

4. **anti_ai_tells** (Cero clichés de IA, Humanizer y Copywriting Confident):
   - 5: 100% Humano, auténtico y en PRIMERA PERSONA SINGULAR ("Diseñé", "Decidí", "Me costó"). Seguro y sin dudas (Confident over qualified: sin "casi", "muy", "bastante", "realmente"). Libre de transiciones mecánicas ("cabe destacar que", "dicho esto", "at its core") y libre de slop ("un testimonio de", "sin fisuras", "game changer", "revolucionario").
   - 3: Tono algo artificial, exceso de calificadores débiles o algún cliché menor.
   - 1: Típico texto generado por IA con clichés promocionales y vacío de sustancia humana.

5. **technical_authority** (Sustancia Técnica y Beneficios sobre Características):
   - 5: Demuestra criterio senior real: conecta la feature/código con su beneficio técnico concreto (latencia, consistencia, memoria, escala). Cero abstracciones vagas: datos y números específicos.
   - 3: Superficial o se limita a listar features sin explicar el beneficio arquitectónico.
   - 1: Sin valor técnico o vago ("optimizamos procesos").

6. **save_and_cta_factor** (Llamado a la Acción de Conversión con Valor):
   - 5: Cierre excelente con fórmula de copywriting: [Verbo de Acción] + [Qué debaten o analizan] + [Pregunta de trade-offs técnicos entre colegas].
   - 3: Cierre aceptable pero algo genérico.
   - 1: CTAs débiles o pasivos prohibidos por copywriting ('Guardá este post', 'Hacé clic aquí', 'Aprendé más', 'Seguime para más').
